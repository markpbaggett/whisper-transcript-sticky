    ...
    return x`
      ...
      <ul>
        ${this.transcript.segments.map(s =>
          x`<whisper-segment .words="${s.words}" start="${s.start}" end="${s.end}" text="${s.text}" />`
        )}
      </ul>
    `;
  }
    ...
    if (this.words) {
      return x`
        <li class="${this.selected ? 'selected' : ''} segment">
          ...
          <div class="words">
          ${this.words.map(w =>
            x`<whisper-word title="${w.probability}" word="${w.word}" start="${w.start}" end="${w.end}" probability="${w.probability}" />`
          )}
          </div>
        </li>
      `;
...
export { WhisperAudio, WhisperSegment, WhisperTranscript, WhisperWord };
